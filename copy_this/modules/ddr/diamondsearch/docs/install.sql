...
CREATE TABLE IF NOT EXISTS `ddrdiamondsearch_term2field` (
  ...
  PRIMARY KEY (`OXID`),
  UNIQUE KEY `DDRTERM2FIELD` (`DDRSHOPID`,`DDRLANGID`,`DDRTERMID`,`DDRFIELD`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `ddrdiamondsearch_term2article` (
  ...
  `DDRLASTINDEXEDAT` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`OXID`),
  UNIQUE KEY `DDRTERM2ARTICLE` (`DDRSHOPID`,`DDRLANGID`,`DDRARTICLEID`,`DDRTERMID`),
  KEY `DDRRELEVANCE` (`DDRRELEVANCE`),
  KEY `DDRTITLEORDER` (`DDRTITLE`(64),`DDRRELEVANCE`),
  KEY `DDRPRICEORDER` (`DDRPRICE`,`DDRRELEVANCE`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
